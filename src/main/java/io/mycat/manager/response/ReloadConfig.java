...
public final class ReloadConfig {
	...
	public static void execute(ManagerConnection c, final boolean loadAll) {
		...
		lock.lock();
		try {
			ListenableFuture<Boolean> listenableFuture = MycatServer
					.getInstance().getListeningExecutorService()
	...
	public static boolean reload_all() {
		...
		/**
		 * 1、载入新的配置 1.1、ConfigInitializer 初始化，基本自检 1.2、DataNode/DataHost 实际链路检测
		 */
		...
		/**
		 * 2、承接 2.1、老的 dataSource 继续承接新建请求 2.2、新的 dataSource 开始初始化， 完毕后交由 2.3
		 * 2.3、新的 dataSource 开始承接新建请求 2.4、老的 dataSource 内部的事务执行完毕， 相继关闭 2.5、老的
		 * dataSource 超过阀值的，强制关闭
		 */
		...
		for (PhysicalDBPool dbPool : newDataHosts.values()) {
			...
			// 获取 data host
			String dnIndex = DnPropertyUtil.loadDnIndexProps().getProperty(
					dbPool.getHostName(), "0");
			if (!"0".equals(dnIndex)) {
		...
		if (isReloadStatusOK) {
			 ...
			 */
			config.reload(newUsers, newSchemas, newDataNodes, newDataHosts,
					newCluster, newFirewall, true);

			/**
			 * 2.4、 处理旧的资源
			 */
			LOGGER.warn("1、clear old backend connection(size): "
			...
			Map<String, PhysicalDBPool> oldDataHosts = config
					.getBackupDataHosts();
			for (PhysicalDBPool dbPool : oldDataHosts.values()) {
				...
				for (PhysicalDatasource ds : dbPool.getAllDataSources()) {
					//
					for (NIOProcessor processor : MycatServer.getInstance()
							.getProcessors()) {
						for (BackendConnection con : processor.getBackends()
								.values()) {
	...
	public static boolean reload() {
		...
		/**
		 * 1、载入新的配置， ConfigInitializer 内部完成自检工作, 由于不更新数据源信息,此处不自检 dataHost
		 * dataNode
		 */
		...
		MycatServer
				.getInstance()
				.getConfig()
				.reload(users, schemas, dataNodes, dataHosts, cluster,
						firewall, false);
