...
public class BatchInsertSequence implements Catlet {
	...
	public void processSQL(String sql, EngineCtx ctx) {
		try {
			...
			LOGGER.error(
					"BatchInsertSequence.processSQL(String sql, EngineCtx ctx)",
					e);
		}
	}
	...
	public void route(SystemConfig sysConfig, SchemaConfig schema, int sqlType,
		...
		try {
			...
			if (insert.getValuesList() != null) {
				String tableName = StringUtil.getTableName(realSQL)
						.toUpperCase();
				...
				if (sequenceHandler == null) {
					int seqHandlerType = MycatServer.getInstance().getConfig()
							.getSystem().getSequnceHandlerType();
					switch (seqHandlerType) {
					case SystemConfig.SEQUENCEHANDLER_MYSQLDB:
						sequenceHandler = IncrSequenceMySQLHandler
								.getInstance();
						break;
				...
				for (ValuesClause vc : insert.getValuesList()) {
					...
					long value = sequenceHandler
							.nextId(tableName.toUpperCase());
	...
	private void getRoute(String sql) {
		try {
			rrs = RouteStrategyFactory.getRouteStrategy().route(sysConfig,
					schema, sqltype, sql, charset, sc, cachePool);
		} catch (Exception e) {
